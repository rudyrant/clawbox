...
func _apply_layout_scaling() -> void:
	...
	var scale_factor: float = clamp(short_side / 360.0, 0.85, 1.9)
	...
	var button_height: float = clamp(56.0 * scale_factor, 48.0, 88.0)
	var button_font: int = int(clamp(20.0 * scale_factor, 16.0, 30.0))
